...
function MessageDetail() {
  ...
  {message && message.length > 0 ? message.map((msg) => (
    <React.Fragment key={msg.id}>
      {/* Show on the left if the message was received by the current user (i.e., sender is not the current user) */}
      {msg.sender.id === user_id &&  
        <div className="chat-message-left pb-4">
          <div>
            <img src={msg.reciever_profile.image} className="rounded-circle mr-1" alt={msg.reciever_profile.full_name} style={{objectFit:"cover"}} width={40} height={40}/>
          </div>
          <div className="message-content flex-shrink-1">
      ...
      {/* Show on the right if the message was sent by the current user */}
      {msg.sender.id !== user_id &&  
        <div className="chat-message-right pb-4">
          <div>
            <img src={msg.sender_profile.image} className="rounded-circle mr-1" alt={msg.sender_profile.full_name} style={{objectFit:"cover"}} width={40} height={40}/>
            <br />
            <div className="text-muted small text-nowrap mt-2">{moment.utc(msg.date).local().startOf('seconds').fromNow()}</div>
          </div>
          <div className="message-content flex-shrink-1">
            <div className="font-weight-bold mb-1">{msg.sender_profile.full_name}</div> {/* Updated the name to 'You' */}
            {msg.message}
          </div>
        </div>
      }
